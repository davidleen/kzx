...
public class KzxMemberLeaderFragment extends BaseFragment {
	...
	public static KzxMemberLeaderFragment newInstance(AmountMemberBean amountMemberBean) {
		KzxMemberLeaderFragment fragment = new KzxMemberLeaderFragment();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tt=new TypeToken<List<MemberLeaderBean>>() {}.getType();
		amountMemberBean=(AmountMemberBean) getArguments().getSerializable("amountMemberBean");
		pastCompanyList.add(new PastCompanyBean(amountMemberBean.getCompanyIcon(), amountMemberBean.getCompanyId(),amountMemberBean.getMemberId(), amountMemberBean.getCompanyName(),amountMemberBean.getTaskLeaderGoodCount(),amountMemberBean.getCompleteLeaderTaskCount(),null));
		for (PastMemberListBean pastMemberListBean : amountMemberBean.getPastMemberList()) {
			pastCompanyList.add(new PastCompanyBean(pastMemberListBean.getCompanyIcon(), pastMemberListBean.getCompanyId(),pastMemberListBean.getMember().getId(), pastMemberListBean.getCompanyName(),pastMemberListBean.getMember().getTaskLeaderGoodCount(),pastMemberListBean.getMember().getCompleteLeaderTaskCount(),null));
		}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		kzxMemberLeaderAdapter=new KzxMemberLeaderAdapter(getActivity());
		...
		dateCustomList.setAdapter(kzxMemberLeaderAdapter);
		dateCustomList=(ListView) view.findViewById(R.id.dateCustomList);
		dateCustomList.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				if (!mHasRequestedMore) {
					...
					if (lastInScreen >= totalItemCount && totalItemCount != 0 && totalItemCount != dateCustomList.getHeaderViewsCount() + dateCustomList.getFooterViewsCount() && kzxMemberLeaderAdapter.getCount() > 0) {
						postLoad(false);
		...
		currentTitle.setText(getString(R.string.account_leader_hint));
		createHeaderViewUI();
	}
...
//		postLoad(true);
	...
	private void createHeaderViewUI(){
		for (final PastCompanyBean pastCompanyBean : pastCompanyList) {
			final View uiView=LayoutInflater.from(getActivity()).inflate(R.layout.kzx_passcompany_leader_header_item_fragment, null);
...
//			Drawable typeImage = getResources().getDrawable(R.drawable.kzx_ic_member_praise_2);
...
//			.into(company_bg);
			...
			amountTv.setText("领导好评:"+pastCompanyBean.getCompanyAmount()+"");
			completeTv.setText("完成任务:"+pastCompanyBean.getCompleteTaskAmount());
...
//					postLoad(true);
	...
	private void postLoad(final boolean isFirstLoad) {
		...
		asyncHttpClient.post(getActivity(), Constants.queryEndIsGoodAPI, rp, new AsyncHttpResponseHandler() {
			@Override
			public void onStart() {
				super.onStart();
				if(isFirstLoad){
					if(kzxMemberLeaderAdapter.getCount()>0){
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
				...
				try {
					...
					if (!StringUtils.isEmpty(records)&&!"[]".equals(records)) {
						List<MemberLeaderBean> noticeList = new ArrayList<MemberLeaderBean>();
						noticeList.addAll((List<MemberLeaderBean>)gson.fromJson(records, tt));
						kzxMemberLeaderAdapter.setDataForLoad(noticeList,isFirstLoad);
