...
public class KzxRegisterFragment extends BaseFragment {
	...
	public static KzxRegisterFragment newInstance(String phone) {
		KzxRegisterFragment fragment = new KzxRegisterFragment();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View contentView = inflater.inflate(R.layout.kzx_register_fragment, null);
		return contentView;
	}
	...
	/** 免注册登录 */
	private void postRegister(final String phone) {
		asyncHttpClient = new AsyncHttpClient();
		RequestParams rp = new RequestParams();
		rp.put("phone", phone);
		asyncHttpClient.setCookieStore(myCookieStore);
		asyncHttpClient.setUserAgent(Constants.USER_AGENT);
		asyncHttpClient.post(getActivity(), Constants.doRegisterByPhoneAPI, rp, responseHandler);
	}
		...
		public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
			...
			try {
				...
				if (data == 2) {
					...
					titleTv.setText(getString(R.string.get_pwd_title));
					if (codeLinear.getVisibility() != View.VISIBLE) {
	...
	private void postVidateCode(final String phone, final String phoneCode) {
		...
		asyncHttpClient.setUserAgent(Constants.USER_AGENT);
		asyncHttpClient.post(getActivity(), Constants.validateCodeByPhoneAPI, rp, codeResponseHandler);
	}

	/** 验证码验证回调 */
	AsyncHttpResponseHandler codeResponseHandler = new AsyncHttpResponseHandler() {
		@Override
		public void onStart() {
			super.onStart();
			showProgressDialog();
	...
	private void postResetPwd(final String data, final String newPwd, final String username) {
		...
		asyncHttpClient.setCookieStore(myCookieStore);
		asyncHttpClient.setUserAgent(Constants.USER_AGENT);
		asyncHttpClient.post(getActivity(), Constants.resetPasswordAPI, rp, pwdResponseHandler);
	}

	/** 验证码验证回调 */
	AsyncHttpResponseHandler pwdResponseHandler = new AsyncHttpResponseHandler() {
		@Override
		public void onStart() {
			super.onStart();
			showProgressDialog();
	...
	private void successMain() {
		...
		if (!StringUtils.isNullOrEmpty(myCookieStore.getCookies())) {
			...
			loginSp.edit().putString("password", "").commit();
		...
		public void onReceive(Context context, Intent intent) {
			if ((getActivity().getPackageName() + ".REGISTER_RECEIVED_ACTION").equals(intent.getAction())) {
				...
				if (smsMessage.indexOf(getString(R.string.app_name)) != -1) {
					phoneCodeTv.setText(smsMessage.substring(smsMessage.lastIndexOf(":") + 1));
					String phoneStr = phoneTv.getText().toString();
					String phoneCodeStr = phoneCodeTv.getText().toString();
					if (StringUtils.isEmpty(phoneCodeStr)) {
						MsgTools.toast(getActivity(), getString(R.string.validate_phone_code), ResourceMap.LENGTH_SHORT);
