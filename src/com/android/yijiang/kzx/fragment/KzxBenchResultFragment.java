...
import com.android.yijiang.kzx.fragment.KzxFaceBackDialogFragment.OnRefreshSchedule;
...
import com.android.yijiang.kzx.widget.listviewanimations.itemmanipulation.swipedismiss.SwipeDismissAdapter;
import com.android.yijiang.kzx.widget.listviewanimations.swinginadapters.prepared.SwingBottomInAnimationAdapter;
import com.android.yijiang.kzx.widget.listviewanimations.widget.DynamicListView;
 ...
 */
@SuppressLint("NewApi")
public class KzxBenchResultFragment extends BaseFragment {
	...
	private EditText filterSearchTv;
	...
	private KzxExecutionAdapter kzxExecutionAdapter;
	...
	public static KzxBenchResultFragment newInstance(String typeStr) {
		KzxBenchResultFragment fragment = new KzxBenchResultFragment();
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		filterSearchTv=(EditText)headerView.findViewById(R.id.filterSearchTv);
		footerView=getLayoutInflater(savedInstanceState).inflate(R.layout.fragment_lv_footer, null);
		footerView.setVisibility(View.INVISIBLE);
		kzxExecutionAdapter=new KzxExecutionAdapter(getActivity(),typeStr);
		...
		//设置滚动动画效果
		final AnimateDismissAdapter animateDismissAdapter = new AnimateDismissAdapter(kzxExecutionAdapter, new MyOnDismissCallback());
		
		final SwingBottomInAnimationAdapter swingBottomInAnimationAdapter = new SwingBottomInAnimationAdapter(animateDismissAdapter);
		...
		kzxExecutionAdapter.setOnItemClickListener(new KzxExecutionAdapter.OnItemClickListener() {
			public void onItemClick(int position, TaskBean taskBean, String typeStr, int type) {
				switch (type) {
				case 0://过
					if("executor".equals(typeStr)){
						List<String> executorIds=(List<String>) aCache.getAsObject(getContext().getACCESS_KEY()+"_executor_passids_"+DateUtil.getCurrentDay());
						if(StringUtils.isNullOrEmpty(executorIds)){
							executorIds=new ArrayList<String>();
						}
						executorIds.remove(taskBean.getId());
						executorIds.add(taskBean.getId());
						aCache.put(getContext().getACCESS_KEY()+"_executor_passids_"+DateUtil.getCurrentDay(),(Serializable)executorIds);
					}else if("sponsor".equals(typeStr)){
						List<String> executorIds=(List<String>) aCache.getAsObject(getContext().getACCESS_KEY()+"_sponsor_passids_"+DateUtil.getCurrentDay());
						if(StringUtils.isNullOrEmpty(executorIds)){
							executorIds=new ArrayList<String>();
						}
						executorIds.remove(taskBean.getId());
						executorIds.add(taskBean.getId());
						aCache.put(getContext().getACCESS_KEY()+"_sponsor_passids_"+DateUtil.getCurrentDay(),(Serializable)executorIds);
					}
					animateDismissAdapter.animateDismiss(position);
					break;
				case 1://反馈
					DialogFragment myFragment = KzxFaceBackDialogFragment.newInstance(taskBean,typeStr,new OnRefreshSchedule() {
						@Override
	...
	private class MyOnDismissCallback implements OnDismissCallback {
		...
		public void onDismiss(AbsListView listView, int[] reverseSortedPositions) {
			for (int position : reverseSortedPositions) {
				kzxExecutionAdapter.clearDataForGuo(position);
				//数据全部过完的时候自动刷新
				if(kzxExecutionAdapter.getCount()==0){
					pageNow = 1;//上拉刷新重置
					postLoad(0);
				}
			}
	...
	private void postLoad(final int isFirstLoad) {
		...
		//获取缓存中的passid(任务ID)
		if("executor".equals(typeStr)){
			passIds=(List<String>) aCache.getAsObject(getContext().getACCESS_KEY()+"_executor_passids_"+DateUtil.getCurrentDay());
		}else if("sponsor".equals(typeStr)){
			passIds=(List<String>) aCache.getAsObject(getContext().getACCESS_KEY()+"_sponsor_passids_"+DateUtil.getCurrentDay());
		}
		...
		if(!StringUtils.isNullOrEmpty(passIds)){
			StringBuffer buffer=new StringBuffer();
			for (String passId : passIds) {
				buffer.append(passId);
				buffer.append(",");
			}
			rp.put("passids", buffer.toString());//过的数据
		}
		asyncHttpClient.post(getActivity(), Constants.taskAPI, rp, new AsyncHttpResponseHandler() {
			@Override
			public void onStart() {
				super.onStart();
				switch (isFirstLoad) {
	...
	public void setUserVisibleHint(boolean isVisibleToUser) {
		...
		if(isVisibleToUser){
			if(kzxExecutionAdapter==null||kzxExecutionAdapter.getCount()==0){
				pageNow=1;
				postLoad(1);
			}
		} 
	...
	private void onRereshTaskBean(TaskIdsBean taskIdsBean){
		if(taskIdsBean!=null){
			System.out.println("shua xin canshu:"+taskIdsBean.getTaskId()+"----"+taskIdsBean.getState());
