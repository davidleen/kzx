...
import com.android.yijiang.kzx.adapter.KzxMemberClientAdapter;
 ...
 */
@SuppressLint("NewApi")
public class KzxMemberClientFragment extends BaseFragment {
	...
	private Gson gson ;
	private Type tt ;
	private KzxMemberClientAdapter kzxMemberClientAdapter;
	...
	public static KzxMemberClientFragment newInstance(AmountMemberBean amountMemberBean) {
		KzxMemberClientFragment fragment = new KzxMemberClientFragment();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tt=new TypeToken<List<MemberClientBean>>() {}.getType();
		amountMemberBean=(AmountMemberBean) getArguments().getSerializable("amountMemberBean");
		pastCompanyList.add(new PastCompanyBean(amountMemberBean.getCompanyIcon(), amountMemberBean.getCompanyId(),amountMemberBean.getMemberId(), amountMemberBean.getCompanyName(),amountMemberBean.getTaskClientGoodCount(),null,amountMemberBean.getCompleteClientTaskCount()));
		for (PastMemberListBean pastMemberListBean : amountMemberBean.getPastMemberList()) {
			pastCompanyList.add(new PastCompanyBean(pastMemberListBean.getCompanyIcon(), pastMemberListBean.getCompanyId(),pastMemberListBean.getMember().getId(), pastMemberListBean.getCompanyName(),pastMemberListBean.getMember().getTaskClientGoodCount(),null,pastMemberListBean.getMember().getCompleteClientTaskCount()));
		}
	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		kzxMemberClientAdapter=new KzxMemberClientAdapter(getActivity());
		...
		dateCustomList.setAdapter(kzxMemberClientAdapter);
		dateCustomList=(ListView) view.findViewById(R.id.dateCustomList);
		dateCustomList.setOnScrollListener(new OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				if (!mHasRequestedMore) {
					...
					if (lastInScreen >= totalItemCount && totalItemCount != 0 && totalItemCount != dateCustomList.getHeaderViewsCount() + dateCustomList.getFooterViewsCount() && kzxMemberClientAdapter.getCount() > 0) {
						postLoad(false);
		...
		currentTitle.setText(getString(R.string.account_client_hint));
		createHeaderViewUI();
	}
...
//		postLoad(true);
	...
	private void createHeaderViewUI(){
		for (final PastCompanyBean pastCompanyBean : pastCompanyList) {
			final View uiView=LayoutInflater.from(getActivity()).inflate(R.layout.kzx_passcompany_client_header_item_fragment, null);
			...
			TextView completeTv=(TextView)uiView.findViewById(R.id.completeTv);
//			ImageView flagIv=(ImageView) uiView.findViewById(R.id.flagIv);
//			Drawable typeImage = getResources().getDrawable(R.drawable.kzx_ic_member_smile_2);
//			typeImage.setBounds(0, 0, typeImage.getMinimumWidth(), typeImage.getMinimumHeight());
//			amountTv.setCompoundDrawables(typeImage, null, null, null);
...
//			.into(company_bg);
			...
			amountTv.setText("客户好评:"+pastCompanyBean.getCompanyAmount()+"");
			completeTv.setText("服务客户:"+pastCompanyBean.getCompleteClientTaskAmount()+"");
...
//					postLoad(true);
	...
	private void postLoad(final boolean isFirstLoad) {
		...
		asyncHttpClient.post(getActivity(), Constants.queryClientEndIsGoodAPI, rp, new AsyncHttpResponseHandler() {
			@Override
			public void onStart() {
				super.onStart();
				if(isFirstLoad){
					if(kzxMemberClientAdapter.getCount()>0){
			...
			public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
				...
				try {
					...
					if (!StringUtils.isEmpty(records)&&!"[]".equals(records)) {
						List<MemberClientBean> noticeList = new ArrayList<MemberClientBean>();
						noticeList.addAll((List<MemberClientBean>)gson.fromJson(records, tt));
						kzxMemberClientAdapter.setDataForLoad(noticeList,isFirstLoad);
