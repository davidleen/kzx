package com.android.yijiang.kzx.widget.fileselector;
...
public class FileSelectionActivity extends Activity {
	...
	Integer[] imageId = { R.drawable.document, R.drawable.document_gray, R.drawable.folder, };
	...
	public void ok() {
		...
		for (int i = 0; i < directoryView.getCount(); i++) {
			if (directoryView.isItemChecked(i)) {
				File file=fileList.get(i - directoryList.size());
				if(FileSizeUtil.getFileOrFilesSize(file, FileSizeUtil.SIZETYPE_MB)>Constants.Filesize){
					MsgTools.toast(this, file.getName()+"文件大小超过了"+Constants.Filesize+"M", 2000);
					isSuccess=false;
					break;
				}else{
		...
		if(resultFileList.size()>3){
			MsgTools.toast(this, "最多只能选择3个附件", 2000);
			return;
		}
